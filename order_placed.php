...
session_start();
include 'connect.php';
$w="select * from orders where customer_id='".($_SESSION["c_id"])."' and address='".($_SESSION["address"])."'";
$re = $con->query($w);
if ($re->num_rows > 0)
...
$rer="select * from order_details where order_id=$id";
$r = $con->query($rer);
?>
<html>
<head>
...
<div style="display:block;width:80%;margin-left:18%;margin-top:9%;background:blue"><h2 style="text-align:center"> <span class="well"style="display:block width:50%">Your Order</span></h2><a style="margin-left:90%"> <span   data-toggle="collapse" data-target="#demo"class=" btn btn-default glyphicon glyphicon-triangle-bottom"></span></h2></a></div>
...
<div class="container">
  ...
  <table class="table table-striped">
    <thead>
	  ...
	  <tr><th>payment_type:</th><th><?php echo $_SESSION['payment_type'];?></th><th></th><th></th></tr>
	  <tr><th>date:</th><th><?php echo $date;?></th><th></th><th></th></tr>
	  <tr><th>address:</th><th><?php echo $_SESSION['address'];?></th></tr>
    </thead>
	<tbody>
	<?php
	...
	{
?>
	<tr><td><?php echo $sno;?></td><?php $idd=$row1['item_id']; $t="select * from menu where ID=$idd";
	$rr = $con->query($t);while($row2 = $rr->fetch_assoc()){$item=$row2['item'];}?><td><?php echo $item;?></td><td><?php echo $row1['quantity']." qty";?></td><td><?php echo $row1['prize'];?></td></tr>
  <?php $sno=$sno+1;
}
?>
